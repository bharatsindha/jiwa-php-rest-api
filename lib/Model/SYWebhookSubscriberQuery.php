...
/**
 * SYWebhookSubscriberQuery Class Doc Comment
 *
 * @category Class
 ...
 * @link     https://github.com/swagger-api/swagger-codegen
 */
class SYWebhookSubscriberQuery implements ModelInterface, ArrayAccess
{
    ...
    /**
      ...
      * @var string
      */
    protected static $swaggerModelName = 'SY_WebhookSubscriberQuery';
    ...
    public function setRowHash($row_hash)
    {
        ...
        if (!is_null($row_hash) && (!preg_match("/^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/", $row_hash))) {
            throw new \InvalidArgumentException("invalid value for $row_hash when calling SYWebhookSubscriberQuery., must conform to the pattern /^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/.");
        }
