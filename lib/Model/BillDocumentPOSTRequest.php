...
/**
 * BillDocumentPOSTRequest Class Doc Comment
 *
 * @category Class
 ...
 * @link     https://github.com/swagger-api/swagger-codegen
 */
class BillDocumentPOSTRequest implements ModelInterface, ArrayAccess
{
    ...
    /**
      ...
      * @var string
      */
    protected static $swaggerModelName = 'BillDocumentPOSTRequest';
    ...
    public function setFileBinary($file_binary)
    {
        ...
        if (!is_null($file_binary) && (!preg_match("/^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/", $file_binary))) {
            throw new \InvalidArgumentException("invalid value for $file_binary when calling BillDocumentPOSTRequest., must conform to the pattern /^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/.");
        }
