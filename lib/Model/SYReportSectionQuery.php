...
/**
 * SYReportSectionQuery Class Doc Comment
 *
 * @category Class
 ...
 * @link     https://github.com/swagger-api/swagger-codegen
 */
class SYReportSectionQuery implements ModelInterface, ArrayAccess
{
    ...
    /**
      ...
      * @var string
      */
    protected static $swaggerModelName = 'SY_ReportSectionQuery';
    ...
    public function setRowHash($row_hash)
    {
        ...
        if (!is_null($row_hash) && (!preg_match("/^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/", $row_hash))) {
            throw new \InvalidArgumentException("invalid value for $row_hash when calling SYReportSectionQuery., must conform to the pattern /^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/.");
        }
